 ...
 */
$data = $_GET['donnees'];
$tabAdherents = json_decode($data);
Adherent::deleteAllAdherents();

foreach ($tabAdherents as $unAdherent) {
    Adherent::addAdherent($unAdherent->numAdherent, $unAdherent->nom, $unAdherent->prenom);
}
$allAdherents = Adherent::getAllAdherents();
echo json_encode($allAdherents);
